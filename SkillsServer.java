...
public class SkillsServer {
    ...
    class ServerThread implements Runnable {
        ...
        void update() throws FileNotFoundException {
            Fridge.configureFromFile("C:\\Users\\mOh\\Documents\\IdeaProjects\\Test\\Fridge.txt");
            Oven.configureFromFile("C:\\Users\\mOh\\Documents\\IdeaProjects\\Test\\Oven.txt");
            Gas.configureFromFile("C:\\Users\\mOh\\Documents\\IdeaProjects\\Test\\Gas.txt");
        }
        ...
        public void run() {
            try {
                ...
                Scanner reader = new Scanner(new File("C:\\Users\\mOh\\Documents\\IdeaProjects\\Test\\StoreCustomers.txt"));
                String temp;
            ...
            while (conversationActive) {
                ...
                try {
                    ...
                    if ((skill.length() == 1) && (skill.toUpperCase().charAt(0) == 'Q'))
                        ...
                        String customerID;
                        if(skill.startsWith("CHECK"))                                                                   // Command: CHECK 100 -> Account Summary for 897
                        {
                        ...
                        if(skill.startsWith("MEMORDER"))                                                                // Command: MEMORDER 897 App 5 Wat 5 Ban 6
                        {
                            ...
                            if (request == null) {
                                ...
                                PrintStream fileStream = new PrintStream(new File("C:\\Users\\mOh\\Documents\\IdeaProjects\\Test\\StoreCustomers.txt"));
                                fileStream.print("");
                                for (int j = 100; j < customers.size(); j++) {

                                    fileStream.println(customers.get(j));
                                }
                                fileStream.println("end");
                                ...
                                if (request == null) {
                                    customers.set(ID, count);
                                    dataout.write(customers.get(ID).toString().getBytes(), 0, customers.get(ID).toString().length());
                                    dataout.write("\n".getBytes(), 0, 1);
                                    dataout.flush();
                                } else {
                                    ...
                                    count = count + customers.get(ID);
                                    customers.set(ID,count);
                                    fileStream = new PrintStream(new File("C:\\Users\\mOh\\Documents\\IdeaProjects\\Test\\StoreCustomers.txt"));
                                    fileStream.print("");
                                    for (int j = 100; j < customers.size(); j++) {
                        ...
                        if(skill.startsWith("FIX"))                                                                     // Command: FIX 897 Fdg CompressorRepair
                        {
                            ...
                            if (AppName.matches("Fdg")) {
                                ...
                                Integer temp = Fridge.FixCostList.get(Fridge.FixList.indexOf(FixName)) + customers.get(ID);
